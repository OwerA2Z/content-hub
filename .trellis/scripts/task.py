...
Archive options:
  ...
  --skip-branch-validation   Archive despite missing or self-referential branch metadata.
                             New tasks with branch metadata tracking enabled refuse a task
                             with no `branch` when it has a `base_branch` and the repo has a
                             remote, or with `branch == base_branch`; repair those with
                             `set-branch` / `set-base-branch` instead. Legacy task records
                             without branch metadata tracking are allowed with a warning.
                             Use this flag only for tasks that were never PR-backed. A
                             recorded branch that was merged and deleted is only a warning.
