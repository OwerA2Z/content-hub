...
export function createPlanningRouter() {
  const router = Router();
  router.get("/strategies", requireAdminOrApiToken, async (_req, res, next) => { try { return res.json({ data: await contentPlanningStore.listStrategies() }); } catch (error) { return next(error); } });
  ...
  router.get("/strategies/:id/series", requireAdminOrApiToken, async (req, res, next) => { try { return res.json({ data: await contentPlanningStore.listSeries(String(req.params.id)) }); } catch (error) { return next(error); } });
  ...
  router.get("/series/:id/briefs", requireAdminOrApiToken, async (req, res, next) => { try { return res.json({ data: await contentPlanningStore.listBriefs(String(req.params.id)) }); } catch (error) { return next(error); } });
