...
# 新任务写入该版本标记，归档时据此区分严格校验与旧格式兼容路径。
BRANCH_METADATA_VERSION = 1
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    task_data = {
        ...
        "completedAt": None,
        "branch": None,
        "branch_metadata_version": BRANCH_METADATA_VERSION,
...
def _validate_branch_metadata(
    ...
    New task records opt into strict branch metadata validation with
    ``branch_metadata_version``. Older records predate this field and are
    allowed to archive with a warning so existing work is not stranded.
    ...
    # Records created before branch tracking was introduced have no version
    # marker; keep them archive-compatible while surfacing the missing data.
    branch_metadata_version = data.get("branch_metadata_version", 0)
    # bool 虽然是 int 的子类，但不能作为有效的元数据版本，避免手改 JSON
    # 用 true 意外绕过新任务的严格分支校验。
    if isinstance(branch_metadata_version, bool) or not isinstance(branch_metadata_version, int):
        branch_metadata_version = 0
    if not branch and base_branch and has_git_remote(repo_root):
        if branch_metadata_version < BRANCH_METADATA_VERSION:
            print(
                colored(
                    f"Warning: task '{task_name}' has no recorded branch, but targets "
                    f"base_branch '{base_branch}'; continuing for compatibility with "
