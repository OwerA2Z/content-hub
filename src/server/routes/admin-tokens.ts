...
export function createAdminTokensRouter() {
  ...
  // 权限更新只修改 scopes，保留原有密文、前缀和创建时间；已撤销 Token 不可重新启用。
  router.patch("/admin/tokens/:id", requireAdminSession, async (req, res, next) => { try { const input = z.object({ scopes: z.array(z.enum(TOKEN_SCOPES)).min(1).max(8) }).parse(req.body); const updated = await tokenStore.updateScopes(String(req.params.id), normalizeScopes(input.scopes)); if (!updated) return sendError(res, 404, "NOT_FOUND", "Token 不存在或已撤销"); await repository.recordAudit({ action: "admin.token.update", actorType: "admin", actorId: res.locals.adminUsername }); return res.json({ data: updated }); } catch (error) { return next(error); } });
